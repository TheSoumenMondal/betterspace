...
export function MonthCalendarLayout() {
	...
	const { data, isLoading, isError } = api.calendar.getEvents.useQuery({
		timeMin,
		timeMax,
		singleEvents: true,
		maxResults: 250,
	...
	const withMeet = allEvents.filter((e: CalendarEvent) => e.hangoutLink).length;
	const withReminders = allEvents.filter(hasReminder).length;
