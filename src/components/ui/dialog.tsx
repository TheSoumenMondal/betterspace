...
const cssAnimationPresets: Record<string, string | string[]> = {
...
const cssTransitionPresets: Record<string, string> = {
...
const DialogContext = createContext<{ modal: boolean } | undefined>(undefined);
...
function Dialog({
	modal = true,
	...props
}: React.ComponentProps<typeof DialogPrimitive.Root> & { modal?: boolean }) {
	return (
...
function DialogViewport(
	props: React.ComponentProps<typeof DialogPrimitive.Viewport>,
) {
...
function DialogPopup({
	...
	animationPreset?: keyof typeof cssAnimationPresets;
	transitionPreset?: keyof typeof cssTransitionPresets;
					...
					<DialogPrimitive.Popup
						...
						render={
							...
							>
								<div
									className={cn(
										"flex w-full flex-col gap-8 overflow-hidden rounded-2xl bg-background px-6 pt-6 pb-0 shadow-lg duration-200",
										containerClassName,
									)}
								>
									{children}
									{showCloseButton && (
										<DialogPrimitive.Close
											...
											render={
												<Button
													className="squircle-none absolute top-6 right-8 -mt-2 -mr-4 size-6 rounded-full p-0"
													size="icon"
													variant="outline"
												>
...
function DialogFooter({
	...
	return (
		<div
			className={cn(
				"mt-auto flex flex-col-reverse gap-2 sm:-mx-6 sm:mt-2 sm:flex-row sm:justify-end sm:rounded-b-2xl sm:border-border/60 sm:border-t sm:bg-muted/50 sm:px-6 sm:py-4",
				className,
			)}
