...
export async function runGmailInitialSync(
	...
	if (nextPageToken && nextLoadedCount < maxEmails) {
		...
		await step.run(`register-gmail-watch-${data.userId}`, async () => {
			try {
				await registerGmailWatch(data.userId);
			} catch (e) {
				console.error("Failed to register Gmail watch during initial sync:", e);
			}
		});
	}
